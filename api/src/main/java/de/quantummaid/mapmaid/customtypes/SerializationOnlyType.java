...
import de.quantummaid.mapmaid.customtypes.serializedobject.serialization_only.SerializationOnlySerializedObject;
import de.quantummaid.mapmaid.identifier.TypeIdentifier;
import de.quantummaid.reflectmaid.GenericType;

import static de.quantummaid.mapmaid.customtypes.serializedobject.serialization_only.SerializationOnlySerializedObject.serializationOnlySerializedObject;
...
public interface SerializationOnlyType<T> extends CustomType<T> {
    ...
    static <C> SerializationOnlyType<C> inlinedCollection(
        ...
        return new SerializationOnlyType<>() {
            @Override
            public <X> X create(final CustomTypeDriver<X> driver) {
                return driver.serializationOnlyCollection(collectionType, contentType, listExtractor);
            }
        };
    }
    ...
    static <T, B> SerializationOnlyType<T> createCustomPrimitive(
        ...
        return new SerializationOnlyType<>() {
            @Override
            public <X> X create(final CustomTypeDriver<X> driver) {
                return driver.serializationOnlyPrimitive(type, serializer, baseType);
            }
        };
    }
}
