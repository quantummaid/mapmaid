package de.quantummaid.mapmaid.validatedtypeskotlin.types

import de.quantummaid.mapmaid.validatedtypeskotlin.validation.StringValidator
import de.quantummaid.mapmaid.validatedtypeskotlin.validation.ValidationHelper.validate

abstract class ValidatedEmail(unsafe: String) : ValidatedString(emailAddress(), unsafe) {
    companion object {
        private val EMAIL_REGEX =
            """(?:[a-z0-9!#${'$'}%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#${'$'}%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])""".toRegex()

        private fun emailAddress(): StringValidator {
            return StringValidator.allOf(
                StringValidator.length(1, 2048), StringValidator.lowercase(), StringValidator {
                    validate(
                        it,
                        EMAIL_REGEX.matches(it)
                    ) { "invalid email address" }
                }
            )
        }
    }
}
