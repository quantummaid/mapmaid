...
public final class FixedValidator implements StructureValidator {
    private final String value;

    public static StructureValidator fixed(final String value) {
        return new FixedValidator(value);
    }

    @Override
    public StructureValidations validate(final Object object) {
        if (!(object instanceof String)) {
            return validation("not a string");
        }
        if (value.equals(object)) {
            return ok();
        } else {
            return validation("not '" + value + "' but '" + object + "'");
        }
    }
