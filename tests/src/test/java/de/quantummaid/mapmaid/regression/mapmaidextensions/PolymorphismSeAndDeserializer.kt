...
open class PolymorphisticCustomMapMaidType<T>(
    ...
    init {
        additionalRequiredTypes.map { TypeIdentifier.typeIdentifierFor(it) }
            .forEach { requiredTypes.add(it) }
    }

    override fun deserializer(): Optional<TypeDeserializer> {
        return Optional.of(PolymorphisticDeserializer(typeToClass, requiredTypes))
    }
    ...
    override fun serializer(): Optional<TypeSerializer> {
        return Optional.of(PolymorphisticSerializer(classToType, requiredTypes))
    }
}
...
private class PolymorphisticSerializer<T>(
    ...
    override fun serialize(
        ...
        val typeIdentifier = TypeIdentifier.typeIdentifierFor(`object`.javaClass)
...
private class PolymorphisticDeserializer<T>(
    ...
    override fun <R : Any?> deserialize(
        ...
        return callback.deserializeRecursive(
            input,
            TypeIdentifier.typeIdentifierFor(kClass),
            exceptionTracker,
            injector,
            debugInformation
        ) as R
