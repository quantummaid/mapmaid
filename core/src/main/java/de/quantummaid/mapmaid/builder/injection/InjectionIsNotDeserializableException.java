...
public final class InjectionIsNotDeserializableException extends MapMaidException {
    ...
    public static InjectionIsNotDeserializableException injectionIsNotDeerializableException(
            final TypeIdentifier typeIdentifier,
            final ScanInformation scanInformation,
            final Object objectToSerialize
    ) {
        final String message = format("Tried to deserialize type '%s' that is marked as injection-only%n%n%s",
                typeIdentifier.description(),
                scanInformation.render()
        );
        return new InjectionIsNotDeserializableException(message, objectToSerialize);
    }
}
